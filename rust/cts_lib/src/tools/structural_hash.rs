...
pub fn structural_hash<'a, V: 'a + Serialize>(v: &V) -> Result<Sha256Hash, String> {
    to_request_id(v)
        .map(|x| *x)
        .map_err(|e| format!("{:?}", e))
}
...
fn to_request_id<'a, V>(value: &V) -> Result<RequestId, RequestIdError>
where
    V: 'a + Serialize,
{
    value
...
struct RequestId(Sha256Hash);

impl RequestId {
    /// Creates a new `RequestId` from a SHA-256 hash.
    fn new(from: &[u8; 32]) -> RequestId {
        RequestId(*from)
    }
}
...
impl Serializer for RequestIdSerializer {
    ...
    // for i128s use the wasabi crate
    fn serialize_i128(self, v: i128) -> Result<Self::Ok, Self::Error> {
        use wasabi_leb128::{WriteLeb128};
        let mut arr = [0u8; 20];
        let n = (&mut &mut arr[..]).write_leb128(v).unwrap();
        Ok(Some(Sha256::digest(&arr[..n]).into()))
    }
    ...
    // for u128s use the wasabi crate
